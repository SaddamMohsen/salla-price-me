...
import { account, merchants, products, user } from './schema';
...
//             .from(account).where(eq(account.user_id, user_id))
...
//         const res = await db.insert(account).values({
...
export const createUser = async (data: any) => {
    try {
        ...
        const result = await db.select({ userId: id }).from(user).where(eq(user.id, id))
